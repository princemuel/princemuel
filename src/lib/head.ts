...
type HeadUserConfig = z.input<ReturnType<typeof MetaSchema>>;
type HeadConfig = z.output<ReturnType<typeof MetaSchema>>;

const HeadSchema = MetaSchema();

/** Create a fully parsed, merged, and sorted head entry array from multiple sources. */
export function createHeadMeta(defaults: HeadUserConfig, ...heads: HeadConfig[]) {
