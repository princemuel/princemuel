		...
		<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
			...
			<body>
				<header id="feed-header">
					...
					<section id="metadata">
						<xsl:choose>
							<xsl:when test="/rss/channel/lastBuildDate"> Last updated at: <time>
									<xsl:value-of select="/rss/channel/lastBuildDate" />
								</time>
							</xsl:when>
							<xsl:otherwise>
								<xsl:if test="/rss/channel/pubDate"> Last published at: <time>
							...
							<xsl:choose>
								<xsl:when test="media:content and media:content/@medium = 'image'">
									<picture>
										<img>
											...
											<xsl:attribute name="width">
												<xsl:value-of select="media:content/@width" />
											</xsl:attribute>
											<xsl:attribute name="height">
												<xsl:value-of select="media:content/@height" />
											</xsl:attribute>
											...
											<xsl:choose>
												...
												<xsl:when test="media:content/@title">
													<xsl:attribute name="alt">
														<xsl:value-of select="media:content/@title" />
													</xsl:attribute>
												</xsl:when>
												<xsl:otherwise>
													<xsl:attribute name="alt">No image description</xsl:attribute>
								...
								<xsl:when test="enclosure">
									<xsl:for-each select="enclosure[position()=1]">
										...
										<xsl:if
											test="$ext = 'jpg' or $ext = 'gif' or $ext = 'png' or $ext = 'jpeg' or $ext = 'webp'">
											<picture>
